...
void GraphicsHelper::drawGlowRect(float x, float y, float w, float h, COLORREF color)
{
...
void GraphicsHelper::drawGlowTriangle(float x, float y, float size, COLORREF color)
{
    BYTE r = GetRValue(color);
    BYTE g = GetGValue(color);
    BYTE b = GetBValue(color);
    ...
    for (int i = 4; i > 0; --i)
    {
        setlinecolor(ARGB(80 - i * 15, r, g, b));
        setlinestyle(PS_SOLID, i * 2);
...
void GraphicsHelper::drawGlowLine(float x1, float y1, float x2, float y2, COLORREF color)
{
    BYTE r = GetRValue(color);
    BYTE g = GetGValue(color);
    BYTE b = GetBValue(color);
...
void GraphicsHelper::drawUI(int score, int playerHp)
{
    ...
    {
        drawGlowRect(float(WINDOW_WIDTH - 110 + i * 25), 12, 20, 15, RGB(255, 0, 255));
    }
}
