...
public:
	...
	//BuffBall(float x, float y): GameObject(x, y, 50.0f, 50.0f), buffColor(RGB(255, 255, 0)), radius(25.0f),showEffect(2) {
	//	// 初始化Buff球的颜色和半径
	//}
	BuffBall(float x, float y);
...
protected:
	...
	Animation<std::function<void()> > showEffect; // Buff球显示效果动画
	void showEffectUpdFunc();
